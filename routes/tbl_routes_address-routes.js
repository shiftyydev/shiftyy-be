...
const {
  ...
  updateRouteAddress,
  deleteRouteAddress,
} = require('../controllers/tbl_routes_address-controller');
...
router.get('/', async (req, res) => {
  getAllRoutesAddresses(req.user, req.query.status)
    .then((result) => res.status(result.status).send(result))
    .catch((error) => {
      sendErrorResp(error, req, res);
    });
...
router.patch('/:id', async (req, res) => {
  updateRouteAddress(req.params.id, req.body)
    .then((result) => res.status(result.status).send(result))
    .catch((error) => {
      sendErrorResp(error, req, res);
    });
