...
const getAllRoutesAddresses = async (user, status) => {
  let routeIds = [];
  ...
  try {
    let whereClause = {
      ...
      [user.userType == 'manager' ? 'route_id' : 'geom']:
    ...
    if (status) {
      whereClause.status = status; // Add filter based on status
    }
    ...
    if (!routeAddresses || routeAddresses.length === 0) {
      return {
        status: 400,
        message: 'No route addresses found',
      };
    } else {
      return {
        status: 200,
